...
| 对象 | 格式示例 | 生成依据 |
| --- | --- | --- |
| dataset_id | ds_household_power_c79e3e19 | 数据别名与源指纹 |
| preprocess_id | prep_c79e3e19_cb65036717cf | dataset_id 与完整处理配置指纹 |
...
M2 已验证：同一源文件和相同配置重复运行时，`dataset_id`、`preprocess_id` 和 64 位 `config_hash` 保持稳定；时间戳不参与数据身份。
...
M2 实际由 `DataService` 编排 `DataCatalog`、`DataValidation`、预处理、manifest 与 SQLite。M4 已在 `powerinsight.forecasting` 实现 `build_windows`：按 split 内部构造窗口，拒绝 NaN、长缺失和 15 分钟不连续，不修改 M2 Parquet。

### 4.4 AnalyticsService

| 操作 | 输入 | 输出 |
| --- | --- | --- |
| inspect_availability | 当前运行上下文 | manifest、数据边界和 ready/blocked 状态 |
| analyze | 开始时间、结束时间上界 | `AnalyticsResult`：范围、KPI、趋势、周期、分项和证据 |

当前实现先校验 manifest schema、`preprocess_id` 目录、15 分钟频率、产物别名、Parquet 必需列和 split 行数，再只读必要列。查询时间范围为半开区间；页面日期两端包含的转换在展示层完成。`representative_days`、相关性和导出仍未实现。
...
当前 Pydantic 契约还包含 `measurement_missing_row_count`，并以 `DataIssue` 与 `MissingBlock` 保存稳定问题代码、严重级别、边界和修复建议。

### 5.2.1 M3 Analytics 对象

`AnalyticsRangeSummary`：

| 字段 | 说明 |
| --- | --- |
| preprocess_id | 当前处理身份 |
| requested_start / requested_end_exclusive | 数据集本地朴素时间的半开查询边界 |
| actual_start / actual_end | 范围中实际存在记录的边界，可为空 |
| expected_points / actual_points | 理论 15 分钟点数与 Parquet 实际记录数 |
| valid_load_points / missing_points | 有效总功率点数与理论缺失点数 |
| coverage_ratio | 有效总功率点数 / 理论点数 |

`AnalyticsKpis` 保持功率与电量分离：累计有功电量为 kWh，平均/峰值/最低有效功率为 kW，峰谷时间可为空，覆盖率与范围摘要一致。

`SubmeterBreakdown` 包含总有功电量、厨房/洗衣房/热水器空调/未分项四个组件、可选占比、负未分项质量记录数和说明。存在负未分项质量证据或无有效正总量时，`shares_available=false`，所有占比为 `null`。

服务内部 `AnalyticsResult` 还携带趋势、小时、星期、工作日/周末 DataFrame、状态、长缺失标记和确定性证据。该对象是进程内服务结果，不写入 SQLite 或业务产物。
...
当前正式默认模型为 `mdl_seasonal_day_m4_20260721_104201`；上例仍只说明请求字段，不应复制示例 ID 覆盖真实选择。
...
~~~text
data/manifests/ds_household_power_c79e3e19.json
data/interim/prep_c79e3e19_cb65036717cf/minute.parquet
data/processed/prep_c79e3e19_cb65036717cf/power_15min.parquet
~~~

M4 当前实际存储：

- `models/checkpoints/m4_20260721_104201/`：Ridge、LSTM、PatchTST 权重，受 `.gitignore` 保护；
- `models/scalers/m4_20260721_104201/target_scaler.json`：仅训练集拟合的缩放器，受 `.gitignore` 保护；
- `models/registry/mdl_*_m4_20260721_104201/`：可提交的小型 model.json、metrics.json、conformal.json 和模型卡；
- `artifacts/forecasts/fcst_*.json/.csv` 与 `artifacts/demo/demo_manifest.json`：离线演示缓存，受 `.gitignore` 保护。

注册记录校验 schema、dataset_id、preprocess_id、数据 config hash、模型 config fingerprint、窗口长度、区间等级、缩放器/权重哈希和代码提交。Ridge、LSTM、PatchTST 在训练脚本结束前必须通过保存/加载推理一致性检查。
...
M2 直接复用 schema v1，没有增加迁移：

- `datasets` 保存数据 ID、非敏感路径别名、SHA-256、行数、时间范围、质量摘要；
- `preprocess_runs` 保存稳定处理 ID、配置指纹、输出别名、状态、统计与耗时；
- 原始分钟和 15 分钟时序均未逐行写入 SQLite；
- 数据集和处理运行使用参数化 SQL 与 upsert，重复运行各保持一条记录；
- 处理异常时先登记 `running`，随后明确更新为 `failed`，不会伪装成 `completed`。
...
| ANALYTICS | 确定性分析 | ANALYTICS_PARQUET_COLUMNS_MISSING |
...
- `ForecastService` 只列出与当前 manifest 和窗口契约兼容的模型；不兼容 schema、身份、配置或哈希直接拒绝。
- forecast request hash 绑定 dataset_id、preprocess_id、model_id、config fingerprint、forecast_start、90% 区间和请求设备。
- 即时结果状态为 `completed`，身份一致的离线复用状态为 `cached`；损坏或身份不一致为 `failed`，数据/模型依赖不存在为 `blocked`。
- 每次预测只把 forecast_id、身份、起点、状态、缓存别名、延迟和时间写入 SQLite；96 点时序保存在文件，不改变 schema v1。
...
| v0.2.0 | 2026-07-21 | 回填 M2 稳定身份、领域对象、文件产物和 schema v1 元数据登记实况 |
| v0.3.0 | 2026-07-21 | 回填 M3 AnalyticsService、范围/KPI/分项对象和脱敏错误契约 |
| v0.4.0 | 2026-07-21 | 回填 M4 正式窗口、模型注册、兼容性、预测缓存和 CSV 导出契约 |
